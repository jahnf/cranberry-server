...
static const struct {
	...
	const char *mimetype;
	const unsigned short mlen;	// mimetype string length
	const unsigned short sss;	// is the extension for server side scripting ?, see __SSSCRIPTING_TYPES?
	const unsigned flags;		// different flags of content type
} extensions [] = {
	// most used extensions up front
    ...
    if( reqinfo->req_method == REQUEST_POST ) {
		...
		if( transfer_encoding ) {
			if( strcmp(transfer_encoding, "chunked") != 0 ) {
				...
				// only chunked is supported, also no mixed transfer encodings.. e.g. gzip, chunked
        ...
        if( !(reqinfo->post_info->flags & REQ_POST_FLAG_TE_CHUNKED) ) {
			if( content_length ) {
				...
				// missing content-length, our server needs this header for post requests,
				// if transfer encoding is not chunked
